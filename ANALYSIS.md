...
| 5 | **Nulls + nomenclator** — some glyphs decode to nothing; frequent words are codebook entries (both documented in Tranchedino's quattrocento cipher ledger) | lowers | implemented: `allow_nulls` (nulls pay 3 bits rent per occurrence plus an output floor; 100% recovery on synthetic null ciphers, nulls identified) and the `nomenclator` hypothesis (substitution + bounded injective codebook with description-length costs; 73% on synthetic nomenclators — code glyphs whose words can pose as cheap one-letter decodes are the residual, an honest objective limitation). On the real manuscript the nomenclator closes ~94% of the gap by assigning frequent Voynich types to Latin function words while spelled words stay unreadable — the familiar pattern: richer key families close more gap without becoming language |
